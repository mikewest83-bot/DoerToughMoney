...
// No NODE_ENV-gated fallback: an unset/misspelled NODE_ENV used to silently
// downgrade this to the hardcoded "dev_only_change_me" secret in production,
// which would let anyone forge a valid session for any account. Require a
// real secret unconditionally; only accept the dev placeholder outside a
// server process entirely (this module still needs *something* so `vitest`
// can import files that reference JWT_SECRET without a .env file).
const JWT_SECRET = process.env.JWT_SECRET || (process.env.VITEST ? "dev_only_change_me" : "");
const TOKEN_TTL = "7d";
