...
> `CHANGES.md`'s top entry for what actually changed. The short version:**
...
>
> **The backend (`server/`) reflects this pivot. The frontend (`web/`) does
> not yet** — it's still built against the old payments API and will not run
> against the new backend without a matching rewrite. See `CHANGES.md`.
